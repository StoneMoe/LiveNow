 ...
 */
export default async function SDPExchange(peerConnection: RTCPeerConnection, endpoint: string) {
  ...
  while (peerConnection.connectionState !== 'closed') {
    ...
    if (response.status === 201) {
      ...
      console.log('invalid WHIP/WHEP URL')
      break
    } else {
      const errorMessage = await response.text()
      console.log(`SDP(${response.status}): ${errorMessage}`)
    }

    /** Limit reconnection attempts to at-most once every 5 seconds */
    await new Promise((r) => setTimeout(r, 5000))
  }
  console.log('giveup SDPOffer requesting')
}
...
async function waitToCompleteICEGathering(peerConnection: RTCPeerConnection) {
  return new Promise<RTCSessionDescription | null>((resolve) => {
    ...
    peerConnection.onicegatheringstatechange = (ev) => peerConnection.iceGatheringState === 'complete' && resolve(peerConnection.localDescription)
  })
}
